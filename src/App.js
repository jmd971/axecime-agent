...
// ── TOKEN avec date de naissance ───────────────────────────────────────────
function hashDob(dob) {
  return btoa(dob.replace(/\D/g,""));
}
function generateToken(dossier) {
  const payload = {
    id:dossier.id, prenom:dossier.prenom, nom:dossier.nom, type:dossier.type,
...
function verifyDob(inputDob, dobHash) {
...
function buildPrompt(type, prenom, pieces) {
  return "Tu es l'assistant IA d'AXECIME, cabinet de courtage indépendant basé en Guadeloupe. Tu t'appelles Alex.\n" +
    ...
    "RÈGLES : Chaleureux, français naturel guadeloupéen. UNE pièce à la fois. Max 2 emojis. Rappelle la progression.\n" +
...
function Bubble({msg}) {
  ...
  return <div style={{display:"flex",justifyContent:agent?"flex-start":"flex-end",marginBottom:8,animation:"fadeUp 0.3s ease"}}>
    ...
    <div style={{maxWidth:"75%"}}>
      ...
      <div style={{background:agent?B.card:"linear-gradient(135deg,"+B.blue+",#0052CC)",color:B.text,padding:"10px 14px",borderRadius:agent?"18px 18px 18px 4px":"18px 18px 4px 18px",fontSize:14,lineHeight:1.65}}>
        {msg.content}
      </div>
...
function DobVerification({prenom, dobHash, onVerified}) {
  ...
  return <div style={{height:"100vh",background:"radial-gradient(ellipse at 30% 40%,"+B.blue+"20 0%,transparent 60%),"+B.dark,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{background:B.surface,borderRadius:20,padding:36,width:380,maxWidth:"100%",border:"1px solid "+B.border,boxShadow:"0 24px 80px rgba(0,0,0,0.5)",animation:shake?"shakeX 0.4s ease":"slideIn 0.4s ease"}}>
      ...
      <div style={{background:B.blue+"18",border:"1px solid "+B.blue+"44",borderRadius:12,padding:"12px 16px",marginBottom:20,textAlign:"center"}}>
        ...
        <div style={{color:B.muted,fontSize:13,marginTop:4}}>Veuillez confirmer votre identité pour accéder à vos documents.</div>
      </div>
      ...
      {blocked ? (
        <div style={{background:"#3F1A1A",border:"1px solid #FF6B6B44",borderRadius:12,padding:16,textAlign:"center"}}>
          ...
          <div style={{color:B.muted,fontSize:12,marginTop:6}}>Trop de tentatives incorrectes. Contactez votre conseiller AXECIME.</div>
        </div>
      ) : (
        <>
          <div style={{marginBottom:16}}>
            <label style={{color:B.muted,fontSize:12,fontWeight:600,display:"block",marginBottom:6}}>VOTRE DATE DE NAISSANCE</label>
            <input
              type="date"
              value={dob}
              ...
              style={{width:"100%",background:B.card,border:"1px solid "+(err?"#FF6B6B":B.border),borderRadius:12,padding:"12px 16px",color:B.text,fontSize:15,boxSizing:"border-box",fontFamily:"'DM Sans',sans-serif",transition:"border-color 0.2s"}}
            />
            {err && <div style={{color:"#FF6B6B",fontSize:12,marginTop:6}}>
              ❌ Date incorrecte — {5-attempts} tentative{5-attempts>1?"s":""} restante{5-attempts>1?"s":""}
            </div>}
          </div>
          <button
            onClick={verify}
            disabled={!dob}
            style={{width:"100%",background:dob?"linear-gradient(135deg,"+B.blue+",#0052CC)":B.border,border:"none",color:dob?"#fff":B.muted,borderRadius:12,padding:14,fontSize:15,fontWeight:700,cursor:dob?"pointer":"not-allowed",fontFamily:"'DM Sans',sans-serif",transition:"all 0.2s"}}>
            Accéder à mes documents →
          </button>
        </>
      )}

      <div style={{textAlign:"center",color:B.muted,fontSize:11,marginTop:16}}>
        🔒 Accès sécurisé · Données chiffrées · Conforme RGPD
      </div>
    </div>
  </div>;
}
...
function ChatView({dossier, onPieceReceived}) {
  ...
  const sendToAgent = useCallback(async (userContent, fileInfo=null, hist=null) => {
    ...
    try {
      ...
      const msgContent = fileInfo ? "[FICHIER: "+fileInfo.name+"] "+(userContent||"Voici le document.") : userContent;
    ...
    <div style={{background:B.surface,borderTop:"1px solid "+B.border,padding:"12px 16px",flexShrink:0}}>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        <input ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{display:"none"}} onChange={e=>{
          ...
          addMsg("client","",fi); sendToAgent("",fi); e.target.value="";
        }}/>
...
function ClientPage({token}) {
  ...
  if (!payload) return <div style={{height:"100vh",background:B.dark,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,padding:24}}>
    ...
    <div style={{color:B.muted,fontSize:14,textAlign:"center",maxWidth:340}}>Ce lien n'est plus valide. Contactez votre conseiller AXECIME pour en recevoir un nouveau.</div>
    <div style={{background:B.surface,borderRadius:12,padding:"12px 20px",border:"1px solid "+B.border,textAlign:"center"}}>
      <div style={{color:B.muted,fontSize:12}}>Contact</div>
      <div style={{color:B.text,fontSize:14,fontWeight:600,marginTop:4}}>axecime.com</div>
    </div>
  </div>;

  const dossier = {
    id:payload.id, prenom:payload.prenom, nom:payload.nom, type:payload.type,
...
// ── DASHBOARD ──────────────────────────────────────────────────────────────
function Dashboard({dossiers, onValidate, onRefuse, onNewDossier, onLogout}) {
  ...
  const copyLink = (dos) => {
    navigator.clipboard.writeText(getClientUrl(dos.token)).then(()=>{
      setCopied(dos.id); setTimeout(()=>setCopied(null),2000);
    });
  };
  ...
  return <div style={{height:"100vh",background:B.dark,display:"flex",flexDirection:"column"}}>
    <div style={{background:B.surface,borderBottom:"1px solid "+B.border,padding:"0 20px",display:"flex",alignItems:"center",height:52,flexShrink:0,gap:12}}>
      ...
      <div style={{marginLeft:"auto",display:"flex",gap:8}}>
        ...
        <button onClick={onLogout} title="Déconnexion" style={{background:B.card,border:"1px solid "+B.border,color:B.muted,borderRadius:8,padding:"6px 10px",fontSize:12,cursor:"pointer"}}>🚪</button>
      </div>
    </div>
    ...
    <div style={{flex:1,display:"flex",overflow:"hidden"}}>
      ...
      <div style={{width:280,background:B.surface,borderRight:"1px solid "+B.border,display:"flex",flexDirection:"column",flexShrink:0}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:8}}>
          {filtered.length===0&&<div style={{color:B.muted,fontSize:13,textAlign:"center",marginTop:24}}>Aucun dossier</div>}
          {filtered.map(dos=>{
      ...
      <div style={{flex:1,overflowY:"auto",background:B.dark}}>
        {d ? (
          <div style={{padding:24,maxWidth:800}}>
            <div style={{background:B.surface,borderRadius:16,padding:20,border:"1px solid "+B.border,marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
              <div>
                ...
                <div style={{color:B.muted,fontSize:13,marginTop:4,display:"flex",gap:12,flexWrap:"wrap"}}>
                  ...
                  {d.dob&&<span>🎂 {new Date(d.dob).toLocaleDateString("fr-FR")}</span>}
                </div>
            ...
            <div style={{background:"#1A3F2F22",border:"1px solid #34D39944",borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
              <span>🔐</span>
              <div style={{color:"#34D399",fontSize:12}}>
                <strong>Accès protégé par date de naissance</strong> — Le client doit saisir sa date de naissance pour accéder au chat.
              </div>
            </div>
            ...
            <div style={{background:B.blue+"15",border:"1px solid "+B.blue+"44",borderRadius:14,padding:16,marginBottom:16}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                {d.email&&<a href={"mailto:"+d.email+"?subject=Vos documents AXECIME&body=Bonjour "+d.prenom+",%0A%0AVoici votre lien sécurisé pour déposer vos documents :%0A%0A"+getClientUrl(d.token)+"%0A%0AVous devrez saisir votre date de naissance pour y accéder.%0ALien valable 30 jours.%0A%0ACordialement,%0AL'équipe AXECIME"}
              ...
              <div style={{color:B.muted,fontSize:11,marginTop:8}}>⏳ 30 jours · 🔐 Protégé par date de naissance</div>
            </div>
              ...
              {d.pieces.map(p=>(
                <div key={p.code} style={{background:B.surface,borderRadius:10,padding:"12px 14px",border:"1px solid "+B.border,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    ...
                    {p.status==="RECU"&&<>
                      <button onClick={()=>onValidate(d.id,p.code)} style={{background:"#1A3F2F",border:"1px solid #34D399",color:"#34D399",borderRadius:8,padding:"4px 10px",fontSize:12,cursor:"pointer",fontWeight:700}}>✓ Valider</button>
                      <button onClick={()=>onRefuse(d.id,p.code)} style={{background:"#3F1A1A",border:"1px solid #FF6B6B",color:"#FF6B6B",borderRadius:8,padding:"4px 10px",fontSize:12,cursor:"pointer",fontWeight:700}}>✕ Refuser</button>
                    </>}
                  </div>
                </div>
              ))}
...
function NewDossierModal({onClose, onCreate}) {
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}} onClick={onClose}>
    <div style={{background:B.surface,borderRadius:20,padding:28,width:420,maxWidth:"100%",border:"1px solid "+B.border,boxShadow:"0 24px 80px rgba(0,0,0,0.6)",animation:"slideIn 0.3s ease",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
      <div style={{color:B.text,fontSize:18,fontWeight:700,marginBottom:20}}>✨ Nouveau Dossier Client</div>

      <div style={{marginBottom:10}}>
        <label style={{color:B.muted,fontSize:12,fontWeight:600,display:"block",marginBottom:4}}>Prénom *</label>
      ...
      <div style={{marginBottom:10}}>
        <label style={{color:B.muted,fontSize:12,fontWeight:600,display:"block",marginBottom:4}}>Nom *</label>
        <input value={form.nom} onChange={e=>set("nom",e.target.value)} placeholder="Dupont"
          style={{width:"100%",background:B.card,border:"1px solid "+B.border,borderRadius:10,padding:"10px 12px",color:B.text,fontSize:14,boxSizing:"border-box"}}/>
      </div>

      <div style={{marginBottom:10}}>
        <label style={{color:"#60A5FA",fontSize:12,fontWeight:600,display:"block",marginBottom:4}}>Date de naissance * (vérification d'accès client)</label>
        <input type="date" value={form.dob} onChange={e=>set("dob",e.target.value)}
          max={new Date().toISOString().split("T")[0]}
          style={{width:"100%",background:B.card,border:"1px solid "+B.blue+"88",borderRadius:10,padding:"10px 12px",color:B.text,fontSize:14,boxSizing:"border-box"}}/>
      </div>

      <div style={{marginBottom:10}}>
        <label style={{color:B.muted,fontSize:12,fontWeight:600,display:"block",marginBottom:4}}>Email</label>
        <input type="email" value={form.email} onChange={e=>set("email",e.target.value)} placeholder="jean@email.com"
          style={{width:"100%",background:B.card,border:"1px solid "+B.border,borderRadius:10,padding:"10px 12px",color:B.text,fontSize:14,boxSizing:"border-box"}}/>
      </div>

      <div style={{marginBottom:10}}>
        <label style={{color:B.muted,fontSize:12,fontWeight:600,display:"block",marginBottom:4}}>Tél WhatsApp (avec indicatif)</label>
        <input type="tel" value={form.tel} onChange={e=>set("tel",e.target.value)} placeholder="+596690000000"
          style={{width:"100%",background:B.card,border:"1px solid "+B.border,borderRadius:10,padding:"10px 12px",color:B.text,fontSize:14,boxSizing:"border-box"}}/>
      </div>

      <div style={{marginBottom:10}}>
        <label style={{color:B.muted,fontSize:12,fontWeight:600,display:"block",marginBottom:4}}>Conseiller assigné</label>
        <input value={form.conseiller} onChange={e=>set("conseiller",e.target.value)} placeholder="Marie L."
          style={{width:"100%",background:B.card,border:"1px solid "+B.border,borderRadius:10,padding:"10px 12px",color:B.text,fontSize:14,boxSizing:"border-box"}}/>
      </div>
      ...
      <div style={{background:B.blue+"18",border:"1px solid "+B.blue+"44",borderRadius:10,padding:"10px 14px",marginBottom:16}}>
        <div style={{color:"#60A5FA",fontSize:12}}>
          🔐 La date de naissance sera utilisée pour vérifier l'identité du client. Elle ne sera jamais visible dans le lien.
        </div>
      </div>

      <div style={{display:"flex",gap:8}}>
        <button onClick={onClose} style={{flex:1,background:B.card,border:"1px solid "+B.border,color:B.muted,borderRadius:10,padding:12,cursor:"pointer",fontWeight:600}}>Annuler</button>
        <button onClick={()=>{ if(valid) onCreate(form); }}
          style={{flex:1,background:valid?"linear-gradient(135deg,"+B.blue+",#0052CC)":B.border,border:"none",color:valid?"#fff":B.muted,borderRadius:10,padding:12,cursor:valid?"pointer":"not-allowed",fontWeight:700}}>
          Créer le dossier ✓
        </button>
      </div>
    </div>
...
// ── APP ────────────────────────────────────────────────────────────────────
const INIT = [
  {id:"dos-001",prenom:"Jean",nom:"Dupont",email:"j.dupont@email.com",tel:"+596690123456",dob:"1985-03-15",type:"PRET_IMMO",statut:"INCOMPLET",conseiller:"Marie L.",pieces:DOSSIER_TYPES.PRET_IMMO.pieces.map((p,i)=>({...p,status:i<3?"VALIDE":i===3?"RECU":"MANQUANT"}))},
  {id:"dos-002",prenom:"Nadège",nom:"Martin",email:"n.martin@email.com",tel:"+596690987654",dob:"1990-07-22",type:"ASSUR_PRET",statut:"EN_COURS",conseiller:"Pierre T.",pieces:DOSSIER_TYPES.ASSUR_PRET.pieces.map((p,i)=>({...p,status:i===0?"VALIDE":"MANQUANT"}))},
  {id:"dos-003",prenom:"Christophe",nom:"Beaumont",email:"c.beaumont@email.com",tel:"+596690554433",dob:"1978-11-05",type:"DEFISC",statut:"COMPLET",conseiller:"Sophie A.",pieces:DOSSIER_TYPES.DEFISC.pieces.map(p=>({...p,status:"VALIDE"}))},
].map(d=>({...d,token:generateToken(d)}));
...
export default function App() {
  ...
  const createDossier = (form) => {
    const d = {id:"dos-"+genId(), ...form, statut:"EN_COURS",
  ...
  return <>
    ...
    />
    {showNew&&<NewDossierModal onClose={()=>setShowNew(false)} onCreate={createDossier}/>}
  </>;
}
