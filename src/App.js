...
// Upload vers SharePoint via /api/upload
async function apiUploadToSharePoint(file, dossierId, nomClient, pieceCode){
...
function buildPrompt(type,prenom,pieces){
  return "Tu es l'assistant IA d'AXECIME, cabinet de courtage indépendant basé en Guadeloupe. Tu t'appelles Alex.\n"+
    ...
    "RÈGLES : Chaleureux, français naturel guadeloupéen. UNE pièce à la fois. Max 2 emojis. Rappelle la progression. Mentionne que les documents sont sauvegardés de façon sécurisée dans SharePoint.\n"+
...
// ── CHAT VIEW avec upload SharePoint ─────────────────────────────────────
function ChatView({dossier,onPieceReceived}){
  ...
  const updateMsgUploadStatus=useCallback((msgId,uploadStatus)=>{
    setMessages(p=>p.map(m=>m.id===msgId?{...m,uploadStatus}:m));
  },[]);
  ...
  const sendToAgent=useCallback(async(userContent,fileInfo=null,hist=null)=>{
    ...
    try{
      ...
      const msgContent=fileInfo?"[FICHIER REÇU: "+fileInfo.name+" — Document bien enregistré dans SharePoint] "+(userContent||"Voici le document demandé."):userContent;
  ...
  const handleFile=async(file)=>{
    // 1. Afficher le fichier avec statut "uploading"
    ...
    let uploadOk=false;
    try{
      await apiUploadToSharePoint(file,dossier.id,dossier.prenom+" "+dossier.nom,pending?.code||"DOC");
      updateMsgUploadStatus(msgId,"success");
      uploadOk=true;
    }catch(e){
      console.error("Erreur SharePoint:",e);
      updateMsgUploadStatus(msgId,"error");
    }

    // 3. Envoyer à l'agent (même si upload échoué, on continue la conversation)
          ...
          <div style={{padding:24,maxWidth:800}}>
            ...
            <div style={{background:"#1A2F3F22",border:"1px solid "+B.blue+"44",borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
              ...
              <div style={{color:"#60A5FA",fontSize:12}}>
                <strong>Stockage SharePoint actif</strong> — Les documents sont automatiquement déposés dans SharePoint AXECIME.
              </div>
            </div>
            ...
            <div style={{background:B.blue+"15",border:"1px solid "+B.blue+"44",borderRadius:14,padding:16,marginBottom:16}}>
              ...
              <div style={{color:B.muted,fontSize:11,marginTop:8}}>⏳ 30 jours · 🔐 Protégé par date de naissance · 📁 Fichiers → SharePoint</div>
            </div>
