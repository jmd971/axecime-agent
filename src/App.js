...
// Appel vers /api/chat (proxy Vercel) — la clé API reste côté serveur
...
// ── VUE CLIENT ─────────────────────────────────────────────────────────────
function ClientView({ dossier, onPieceReceived }) {
  ...
  const sendToAgent = useCallback(async (userContent, fileInfo = null, hist = null) => {
    ...
    try {
      const pieces = dossier.pieces.map(p => ({ ...p }));
      ...
      const reply = await callAgent(updated, SYSTEM_PROMPT(dossier.type, dossier.prenom, pieces));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      ).catch(() => `Bonjour ${dossier.prenom} ! 👋 Je suis Alex, votre assistant AXECIME. Commençons par votre carte d'identité : pouvez-vous m'envoyer une photo recto-verso ?`);
      setTyping(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ background: B.surface, borderBottom: `1px solid ${B.border}`, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: "#34D399", fontSize: 12, display: "flex", alignItems: "center", gap: 4 }}>
            ...
            En ligne · Assistant IA certifié
          </div>
        </div>
      ...
      <div style={{ background: `linear-gradient(90deg,${B.blue}18,${B.orange}18)`, borderBottom: `1px solid ${B.border}`, padding: "7px 16px", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        <span style={{ fontSize: 16 }}>{DOSSIER_TYPES[dossier.type].icon}</span>
      ...
      <div style={{ background: B.surface, borderTop: `1px solid ${B.border}`, padding: "12px 16px", flexShrink: 0 }}>
        ...
        <div style={{ textAlign: "center", color: B.muted, fontSize: 11, marginTop: 8 }}>🔒 Documents chiffrés · SharePoint · Conforme RGPD</div>
      </div>
    </div>
  );
}
        ...
        {sel ? (() => {
          ...
          return (
            <div style={{ padding: 24 }}>
              <div style={{ background: B.surface, borderRadius: 16, padding: 20, border: `1px solid ${B.border}`, marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
...
// ── APP PRINCIPALE ─────────────────────────────────────────────────────────
const INIT = [
  { id: "dos-001", prenom: "Jean", nom: "Dupont", email: "j.dupont@email.com", tel: "+596 690 12 34 56", type: "PRET_IMMO", statut: "INCOMPLET", conseiller: "Marie L.", pieces: DOSSIER_TYPES.PRET_IMMO.pieces.map((p, i) => ({ ...p, status: i < 3 ? "VALIDE" : i === 3 ? "RECU" : "MANQUANT" })) },
  { id: "dos-002", prenom: "Nadège", nom: "Martin", email: "n.martin@email.com", tel: "+596 690 98 76 54", type: "ASSUR_PRET", statut: "EN_COURS", conseiller: "Pierre T.", pieces: DOSSIER_TYPES.ASSUR_PRET.pieces.map((p, i) => ({ ...p, status: i === 0 ? "VALIDE" : "MANQUANT" })) },
  { id: "dos-003", prenom: "Christophe", nom: "Beaumont", email: "c.beaumont@email.com", tel: "+596 690 55 44 33", type: "DEFISC", statut: "COMPLET", conseiller: "Sophie A.", pieces: DOSSIER_TYPES.DEFISC.pieces.map(p => ({ ...p, status: "VALIDE" })) },
];
...
export default function App() {
  ...
  const updatePiece = (dossierId, pieceCode, newStatus) => {
    ...
    setDossiers(up);
    setActive(prev => up([prev])[0]);
  };

  const createDossier = () => {
    if (!form.prenom || !form.nom) return;
    const d = { id: `dos-${genId()}`, conseiller: "Non assigné", ...form, statut: "EN_COURS", pieces: DOSSIER_TYPES[form.type].pieces.map(p => ({ ...p, status: "MANQUANT" })) };
    setDossiers(p => [d, ...p]);
    setActive(d);
    setShowNew(false);
  ...
  return (
    <div style={{ height: "100vh", background: B.dark, display: "flex", flexDirection: "column" }}>
      <div style={{ background: B.surface, borderBottom: `1px solid ${B.border}`, padding: "0 20px", display: "flex", alignItems: "center", height: 52, flexShrink: 0 }}>
        ...
        {[{ id: "dashboard", label: "📊 Dashboard" }, { id: "client", label: "💬 Portail Client" }].map(t => (
      ...
      <div style={{ flex: 1, overflow: "hidden" }}>
        {view === "dashboard" ? (
          <Dashboard dossiers={dossiers} onSelectDossier={setActive} onValidate={(id, code) => updatePiece(id, code, "VALIDE")} onRefuse={(id, code) => updatePiece(id, code, "REFUSE")} />
        ) : (
          <div style={{ display: "flex", height: "100%" }}>
            <div style={{ width: 200, background: B.surface, borderRight: `1px solid ${B.border}`, overflowY: "auto", padding: 8, flexShrink: 0 }}>
              ...
              {dossiers.map(d => (
                <div key={d.id} onClick={() => setActive(d)} style={{ background: active?.id === d.id ? `${B.blue}22` : "transparent", border: `1px solid ${active?.id === d.id ? B.blue : "transparent"}`, borderRadius: 10, padding: "9px 10px", cursor: "pointer", marginBottom: 3, transition: "all 0.2s" }}>
            ...
            <div style={{ flex: 1 }}>
              {active && <ClientView key={active.id} dossier={active} onPieceReceived={code => updatePiece(active.id, code, "RECU")} />}
            </div>
          </div>
        )}
      </div>
      ...
      {showNew && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setShowNew(false)}>
          <div style={{ background: B.surface, borderRadius: 20, padding: 28, width: 420, border: `1px solid ${B.border}`, boxShadow: "0 24px 80px rgba(0,0,0,0.6)", animation: "slideIn 0.3s ease" }} onClick={e => e.stopPropagation()}>
            ...
            {[{ k: "prenom", l: "Prénom", ph: "Jean" }, { k: "nom", l: "Nom", ph: "Dupont" }, { k: "email", l: "Email", ph: "jean@email.com" }, { k: "tel", l: "Téléphone WhatsApp", ph: "+596 690 00 00 00" }, { k: "conseiller", l: "Conseiller assigné", ph: "Marie L." }].map(f => (
