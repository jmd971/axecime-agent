...
function Spinner({text="Chargement..."}) {
  return <div style={{height:"100%",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,color:B.muted}}>
    <div style={{width:36,height:36,borderRadius:"50%",border:"3px solid "+B.border,borderTopColor:B.blue,animation:"spin 0.8s linear infinite"}}/>
      ...
      {blocked?(
        ...
        <>
          <div style={{marginBottom:16}}>
            ...
            <input type="date" value={dob} onChange={e=>{setDob(e.target.value);setErr(false);}}
            ...
            {err&&<div style={{color:"#FF6B6B",fontSize:12,marginTop:6}}>❌ Date incorrecte — {5-attempts} tentative{5-attempts>1?"s":""} restante{5-attempts>1?"s":""}</div>}
          </div>
          <button onClick={verify} disabled={!dob}
            style={{width:"100%",background:dob?"linear-gradient(135deg,"+B.blue+",#0052CC)":B.border,border:"none",color:dob?"#fff":B.muted,borderRadius:12,padding:14,fontSize:15,fontWeight:700,cursor:dob?"pointer":"not-allowed",fontFamily:"'DM Sans',sans-serif"}}>
...
function ChatView({dossier,onPieceReceived}) {
  ...
  const sendToAgent=useCallback(async(userContent,fileInfo=null,hist=null)=>{
    ...
    }catch{addMsg("agent","Désolé, une erreur est survenue. Réessayez dans un instant 🙏");}
...
// ── DASHBOARD ──────────────────────────────────────────────────────────────
function Dashboard({dossiers,loading,onValidate,onRefuse,onNewDossier,onLogout,onRefresh}) {
  ...
  const copyLink=(dos)=>{
    navigator.clipboard.writeText(getClientUrl(dos.token)).then(()=>{setCopied(dos.id);setTimeout(()=>setCopied(null),2000);});
  };

  const handlePiece=async(dossierId,pieceCode,newStatus)=>{
    setSaving(pieceCode);
    try{
      await onValidate(dossierId,pieceCode,newStatus);
        ...
        {d?(
          <div style={{padding:24,maxWidth:800}}>
            ...
            <div style={{background:"#1A3F2F22",border:"1px solid #34D39944",borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
              <span>🔐</span>
              <div style={{color:"#34D399",fontSize:12}}><strong>Accès protégé par date de naissance</strong> — Le client doit saisir sa date de naissance pour accéder au chat.</div>
            </div>
            ...
            <div style={{background:B.blue+"15",border:"1px solid "+B.blue+"44",borderRadius:14,padding:16,marginBottom:16}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                {d.email&&<a href={"mailto:"+d.email+"?subject=Vos documents AXECIME&body=Bonjour "+d.prenom+",%0A%0AVoici votre lien sécurisé :%0A%0A"+getClientUrl(d.token)+"%0A%0AVous devrez saisir votre date de naissance.%0ALien valable 30 jours.%0A%0ACordialement,%0AL'équipe AXECIME"}
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}} onClick={onClose}>
    <div style={{background:B.surface,borderRadius:20,padding:28,width:420,maxWidth:"100%",border:"1px solid "+B.border,boxShadow:"0 24px 80px rgba(0,0,0,0.6)",animation:"slideIn 0.3s ease",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{background:B.blue+"18",border:"1px solid "+B.blue+"44",borderRadius:10,padding:"10px 14px",marginBottom:16}}>
        <div style={{color:"#60A5FA",fontSize:12}}>🔐 La date de naissance protège l'accès client. Elle ne sera jamais visible dans le lien.</div>
      </div>
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={()=>{if(valid&&!saving)onCreate(form);}}
...
export default function App() {
  ...
  // Route client → pas de chargement BD nécessaire
  ...
  // Chargement des dossiers depuis Vercel KV
  ...
  return <>
    ...
    <Dashboard
      ...
      onRefuse={(id,code)=>handleValidate(id,code,"REFUSE")}
